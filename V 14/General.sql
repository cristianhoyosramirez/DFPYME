...
update configuracion_pedido set texto_propina ='Este establecimiento sugiere un aporte de servicio voluntario del 10% del valor de la cuenta. Usted puede aceptarlo, rechazarla o modificarlo según su valoración del servicio. 
...
DO $$
BEGIN
    -- Verificar si ya existe el departamento con iddepartamento = 760
    IF NOT EXISTS (
        SELECT 1
        FROM departamento
        WHERE iddepartamento = 760
    ) THEN
        -- Insertar solo si no existe
        INSERT INTO departamento (iddepartamento, nombre, region, codigo)
        VALUES (760, 'BOGOTÁ D.C.', 49, '11');
    END IF;
END$$;

UPDATE ciudad SET iddepartamento = 760, numero = '001', nombreciudad = 'BOGOTÁ D.C.' 
  WHERE idciudad = 462;


-- Column: calculo_propina

-- ALTER TABLE configuracion_pedido DROP COLUMN calculo_propina;

DO $$
BEGIN
    -- Si la columna no existe, se crea con default false
    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'configuracion_pedido'
          AND column_name = 'calculo_propina'
    ) THEN
        ALTER TABLE configuracion_pedido
        ADD COLUMN calculo_propina boolean DEFAULT false;
    END IF;

    -- Siempre asegurar que el default quede en false
    ALTER TABLE configuracion_pedido
    ALTER COLUMN calculo_propina SET DEFAULT false;
END$$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_attribute
        WHERE attrelid = 'configuracion_pedido'::regclass
          AND attname = 'preguntar_impresora_prefactura'
          AND NOT attisdropped
    ) THEN
        ALTER TABLE configuracion_pedido
        ADD COLUMN preguntar_impresora_prefactura boolean DEFAULT false;
    END IF;
END$$;

update configuracion_pedido set preguntar_impresora_prefactura='false';
...
BEGIN
    IF NOT EXISTS (
        ...
        FROM pg_attribute
        WHERE attrelid = 'configuracion_pedido'::regclass
          AND attname = 'terminos_condiones'
          AND NOT attisdropped
    ) THEN
        ALTER TABLE configuracion_pedido
        ADD COLUMN terminos_condiones text;
    END IF;
END$$;
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        FROM information_schema.columns
        WHERE table_name = 'configuracion_pedido'
          AND column_name = 'facturar_cero'
    ) THEN
        ALTER TABLE configuracion_pedido ADD COLUMN facturar_cero boolean DEFAULT true;
    END IF;

    -- Si ya existe, asegurar que el default sea true
    ALTER TABLE configuracion_pedido ALTER COLUMN facturar_cero SET DEFAULT true;
END$$;
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        FROM information_schema.columns 
        WHERE table_name='configuracion_pedido' 
          AND column_name='reimpresion_meseros'
    ) THEN
        -- Crear columna si no existe
        ALTER TABLE configuracion_pedido ADD COLUMN reimpresion_meseros boolean DEFAULT false;
    ELSE
        -- Si ya existe, asegurar el DEFAULT
        ALTER TABLE configuracion_pedido ALTER COLUMN reimpresion_meseros SET DEFAULT false;
    END IF;
END;
$$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_attribute
        WHERE attrelid = 'productos_borrados'::regclass
          AND attname = 'justificacion'
          AND NOT attisdropped
    ) THEN
        ALTER TABLE productos_borrados
        ADD COLUMN justificacion text;
    END IF;
END$$;


INSERT INTO estado (idestado, descripcionestado, estado, orden, consulta)
SELECT v.idestado,
       v.descripcionestado,
       CASE WHEN v.idestado = 7 THEN FALSE ELSE v.estado END AS estado,
       v.orden,
       CASE WHEN v.idestado = 7 THEN FALSE ELSE v.consulta END AS consulta
FROM (
    VALUES
        (1, 'FACTURA POS CONTADO', FALSE, 1, FALSE),
        (2, 'CRÉDITO', FALSE, 4, FALSE),
        (3, 'PENDIENTE', FALSE, 5, FALSE),
        (4, 'COTIZACIÓN', FALSE, 7, FALSE),
        (5, 'TODOS LOS CRITERIOS', FALSE, 6, FALSE),
        (6, 'CORTESIA', TRUE, 9, TRUE),
        (7, 'REMISIÓN CONTADO', FALSE, 3, FALSE), -- aquí igual, pero abajo lo forzamos
        (8, 'FACTURA ELECTRÓNICA', TRUE, 2, TRUE),
        (9, 'CREDITO CON SALDO', FALSE, 8, FALSE)
) AS v(idestado, descripcionestado, estado, orden, consulta)
WHERE NOT EXISTS (
    SELECT 1 FROM estado e WHERE e.idestado = v.idestado
);


update estado set descripcionestado = 'FACTURA POS CONTADO' where idestado = 1;
update estado set descripcionestado = 'CRÉDITO' where idestado = 2;
update estado set descripcionestado = 'PENDIENTE' where idestado = 3;
update estado set descripcionestado = 'COTIZACIÓN' where idestado = 4;
update estado set descripcionestado = 'TODOS LOS CRITERIOS' where idestado = 5;
update estado set descripcionestado = 'CORTESIA' where idestado = 6;
update estado set descripcionestado = 'REMISIÓN CONTADO' where idestado = 7;
update estado set descripcionestado = 'FACTURA ELECTRÓNICA' where idestado = 8;
update estado set descripcionestado = 'CREDITO CON SALDO' where idestado = 9;

update estado set estado ='FALSE'where idestado= 1;				
update estado set estado ='FALSE'where idestado= 2;				
update estado set estado ='FALSE'where idestado= 3;				
update estado set estado ='FALSE'where idestado= 4;				
update estado set estado ='FALSE'where idestado= 5;				
update estado set estado ='TRUE'where idestado= 6;				
update estado set estado ='FALSE'where idestado= 7;				
update estado set estado ='TRUE'where idestado= 8;				
update estado set estado ='FALSE'where idestado= 9;

update estado set orden = 1 where idestado= 1;
update estado set orden = 4 where idestado= 2;
update estado set orden = 5 where idestado= 3;
update estado set orden = 7 where idestado= 4;
update estado set orden = 6 where idestado= 5;
update estado set orden = 9 where idestado= 6;
update estado set orden = 3 where idestado= 7;
update estado set orden = 2 where idestado= 8;
update estado set orden = 8 where idestado= 9;

update estado set consulta ='FALSE'where idestado= 1;
update estado set consulta ='FALSE'where idestado= 2;
update estado set consulta ='FALSE'where idestado= 3;
update estado set consulta ='FALSE'where idestado= 4;
update estado set consulta ='FALSE'where idestado= 5;
update estado set consulta ='TRUE'where idestado= 6;
update estado set consulta ='FALSE'where idestado= 7;
update estado set consulta ='TRUE'where idestado= 8;
update estado set consulta ='FALSE'where idestado= 9;

UPDATE productos_borrados pb
SET 
    id_mesero = COALESCE(pb.id_mesero, 6),
    valor_unitario = COALESCE(pb.valor_unitario, p.valorventaproducto)
FROM producto p
WHERE pb.codigointernoproducto = p.codigointernoproducto
  AND (pb.id_mesero IS NULL OR pb.valor_unitario IS NULL);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_name = 'pedidos_borrados'
...
UPDATE configuracion_pedido SET version = 14;
