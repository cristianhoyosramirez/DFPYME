async function buscarCiudadDestino(valor) {
    try {
        
        //const response = await fetch(base_url + '/habitaciones/buscarCliente', {
        const response = await fetch(`${BASE_URL}/habitaciones/buscarCiudadDestino`, {
            method: 'POST',
            headers: {
        ...
        const lista = document.getElementById('listaMunicipiosDestino');
