...
class ConfigurarProductocontroller extends BaseController
{
    ...
    function getAtributosProducto()
    {
        ...
        if ($contarComponentes[0]['numero_componentes'] < $numeroComponentes[0]['numero_componentes']) {
            ...
            if ($insert) {
                ...
                $tempAtributos = model('atributosDeProductoModel')->getAtributos($id_tabla_producto);
    ...
    function deleteComponenteProducto()
    {
        ...
        $json = $this->request->getJSON();
        $id_componente = $json->id;
        $tempTablaProducto = model('atributosDeProductoModel')->select('id_tabla_producto')->where('id', $id_componente)->first();
        $id_tabla_producto = $tempTablaProducto['id_tabla_producto'];
    ...
    public function adicionDeProducto()
    {
        ...
        $total_pedido = $pedidoModel->select('valor_total,fecha_creacion')->where('id', $idPedido)->first();
