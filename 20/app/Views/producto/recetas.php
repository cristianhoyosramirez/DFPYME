...
<script>
    async function actualizarCosto(valor, codigointerno, id) {
        ...
        try {
            ...
            // Asignamos valor
            rentabilidadInput.value = data.rentabilidad.replace(/<[^>]+>/g, ''); // eliminamos cualquier HTML si viene del backend

            // Limpiamos clases anteriores
            rentabilidadInput.classList.remove('text-success', 'text-danger');

            // Convertimos a número para evaluar
            const rentNum = parseFloat(data.rentabilidad.replace(/\./g, '').replace(',', '.').replace(/<[^>]+>/g, ''));

            // Pintamos según valor
            if (rentNum >= 0) {
                rentabilidadInput.classList.add('text-success'); // verde
            } else {
                rentabilidadInput.classList.add('text-danger'); // rojo
            }
