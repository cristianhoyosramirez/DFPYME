...
class cajaGeneralController extends BaseController
{
    ...
    public function exportProductosBorradosExcel()
    {
        $fecha_inicial = $this->request->getPost('fecha_inicial');
        $fecha_final   = $this->request->getPost('fecha_final');
        ...
        $file_name = 'Reporte_productos_borrados_' . $fecha_inicial . '_al_' . $fecha_final . '.xlsx';
        ...
        $sheet->setCellValue(
            'A4',
            'Dirección: ' . $datos_empresa[0]['direccionempresa'] . ' ' .
                $datos_empresa[0]['nombreciudad'] . ' ' .
                $datos_empresa[0]['nombredepartamento']
        );
        $sheet->mergeCells('A4:I4');

        $sheet->setCellValue('A5', 'REPORTE PRODUCTOS BORRADOS');
        ...
        // CABECERA TABLA
        // =============================
        $sheet->setCellValue('A8', 'Pedido');
        $sheet->setCellValue('B8', 'Código');
        $sheet->setCellValue('C8', 'Producto');
        $sheet->setCellValue('D8', 'Cantidad');
        $sheet->setCellValue('E8', 'Fecha');
        $sheet->setCellValue('F8', 'Hora');
        $sheet->setCellValue('G8', 'Usuario creación');
        $sheet->setCellValue('H8', 'Usuario eliminación');
        $sheet->setCellValue('I8', 'Justificación');
        ...
        foreach ($productos as $detalle) {
            ...
            $sheet->setCellValue("A$row", $detalle['pedido']);
            $sheet->setCellValue("B$row", $detalle['codigointernoproducto']);
            $sheet->setCellValue("C$row", $detalle['nombreproducto']);
            $sheet->setCellValue("D$row", $detalle['cantidad']);

            // Fecha formateada
            $fecha = date("d/m/Y", strtotime($detalle['fecha_eliminacion']));
            $sheet->setCellValue("E$row", $fecha);

            // Hora formateada
            $hora = date("h:i A", strtotime($detalle['hora_eliminacion']));
            $sheet->setCellValue("F$row", $hora);

            $sheet->setCellValue("G$row", $detalle['nombresusuario_sistema']);
            $sheet->setCellValue("H$row", $detalle['mesero_nombre']);
            $sheet->setCellValue("I$row", $detalle['justificacion']);
    ...
    function exportVentas()
    {
        ...
        foreach ($invoices as $detalle) {
            ...
            //$sheet->setCellValue("E$row", $documento['numero']);
            //d($documento['numero']);
            if (is_array($documento) && isset($documento['numero'])) {
                $sheet->setCellValue("E$row", $documento['numero']);
            } else {
                $sheet->setCellValue("E$row", 'Dato inválido');
            }
    ...
    function exportFormasPagoExcel()
    {
        $fecha_inicial = $this->request->getPost('fecha_inicial');
        $fecha_final = $this->request->getPost('fecha_final');
        ...
        $file_name = 'Reporte_Formas_de_Pago_' . $fecha_inicial . '_al_' . $fecha_final . '.xlsx';
        ...
        $direccion = 'Dirección: ' . $datos_empresa[0]['direccionempresa'] . ", " . $datos_empresa[0]['nombreciudad'] . " - " . $datos_empresa[0]['nombredepartamento'];
        ...
        $sheet->setCellValue('A5', 'REPORTE DE FORMAS DE PAGO');
        ...
        $sheet->setCellValue('A8', 'N° Pedido');
        $sheet->setCellValue('B8', 'Fecha');
        $sheet->setCellValue('C8', 'Hora');
        $sheet->setCellValue('D8', 'Factura');
        $sheet->setCellValue('E8', 'Valor Documento');
        $sheet->setCellValue('F8', 'Pago Efectivo');
        $sheet->setCellValue('G8', 'Pago Banco');
        $sheet->setCellValue('H8', 'Clase de Pago');
        ...
        foreach ($formasPago as $detalle) {
            $fecha = date('d/m/Y', strtotime($detalle['fecha']));
            $hora = date("h:i A", strtotime($detalle['hora']));

            $medioPago = 'Efectivo';
            if (!empty($detalle['id_clase_pago'])) {
                $nombreMedio = model('clasePagoModel')->select('nombre')->where('id', $detalle['id_clase_pago'])->first();
                $medioPago = $nombreMedio ? $nombreMedio['nombre'] : 'Efectivo';
            }

            $valorDocumento = floatval($detalle['total_documento']);
            $efectivo = floatval($detalle['efectivo']);
            $banco = floatval($detalle['transferencia']);

            $sheet->setCellValue("A$row", $detalle['id_pedido']);
            $sheet->setCellValue("B$row", $fecha);
            $sheet->setCellValue("C$row", $hora);
            $sheet->setCellValue("D$row", $detalle['numero']);
            $sheet->setCellValue("E$row", $valorDocumento);
            $sheet->setCellValue("F$row", $efectivo);
            $sheet->setCellValue("G$row", $banco);
            $sheet->setCellValue("H$row", $medioPago);

            $totalDocumento += $valorDocumento;
            $totalEfectivo += $efectivo;
            $totalBanco += $banco;

            if (!isset($resumenPagos[$medioPago])) {
                $resumenPagos[$medioPago] = 0;
            }
            $resumenPagos[$medioPago] += $valorDocumento;
        ...
        $sheet->setCellValue("E$row", $totalDocumento);
        $sheet->setCellValue("F$row", $totalEfectivo);
        $sheet->setCellValue("G$row", $totalBanco);
        $sheet->getStyle("D$row:H$row")->applyFromArray($headerStyle);
    ...
    public function exportPedidosBorrados()
    {
        $fecha_inicial = $this->request->getPost('fecha_inicial');
        $fecha_final   = $this->request->getPost('fecha_final');

        $model = model('eliminacionPedidosModel');

        if (!empty($fecha_inicial) && !empty($fecha_final)) {
            $pedidos_borrados = $model
        ...
        $file_name = 'Reporte_pedidos_borrados_' . $fecha_inicial . '_al_' . $fecha_final . '.xlsx';
        ...
        $sheet->setCellValue('A1', $datos_empresa[0]['nombrejuridicoempresa']);
        $sheet->mergeCells('A1:H1');
        $sheet->getStyle('A1')->applyFromArray($headerEmpresaStyle);

        $sheet->setCellValue('A2', $datos_empresa[0]['nombrecomercialempresa']);
        $sheet->mergeCells('A2:H2');
        $sheet->getStyle('A2')->applyFromArray($headerEmpresaStyle);

        $sheet->setCellValue('A3', 'NIT: ' . $datos_empresa[0]['nitempresa']);
        $sheet->mergeCells('A3:H3');
        $sheet->getStyle('A3')->applyFromArray($headerEmpresaStyle);

        $sheet->setCellValue('A4', 'Dirección: ' . $datos_empresa[0]['direccionempresa']);
        $sheet->mergeCells('A4:H4');
        $sheet->getStyle('A4')->applyFromArray($headerEmpresaStyle);

        $sheet->setCellValue('A5', 'REPORTE DE PEDIDOS ELIMINADOS');
        $sheet->mergeCells('A5:H5');
        $sheet->getStyle('A5')->applyFromArray($tituloStyle);
        ...
        // ENCABEZADOS TABLA
        // =============================
        $sheet->setCellValue('A8', 'Fecha eliminación');
        $sheet->setCellValue('B8', 'Hora');
        $sheet->setCellValue('C8', 'Fecha creación');
        $sheet->setCellValue('D8', 'Pedido');
        $sheet->setCellValue('E8', 'Valor');
        $sheet->setCellValue('F8', 'Usuario creación');
        $sheet->setCellValue('G8', 'Usuario eliminación');
        $sheet->setCellValue('H8', 'Justificación');

        $sheet->getStyle('A8:H8')->applyFromArray($headerTablaStyle);
        ...
        foreach ($pedidos_borrados as $detalle) {
            ...
            $nombre_creacion = $usuarioModel
                ->select('nombresusuario_sistema')
                ->where('idusuario_sistema', $detalle['id_mesero'])
                ->first();

            $nombre_eliminacion = $usuarioModel
                ->select('nombresusuario_sistema')
                ->where('idusuario_sistema', $detalle['usuario_eliminacion'])
                ->first();

            $sheet->setCellValue("A$row", $detalle['fecha_eliminacion']);
            $sheet->setCellValue("B$row", $detalle['hora_eliminacion']);
            $sheet->setCellValue("C$row", $detalle['fecha_creacion']);
            $sheet->setCellValue("D$row", $detalle['numero_pedido']);
            $sheet->setCellValue("E$row", $detalle['valor_pedido']);
            $sheet->setCellValue("F$row", $nombre_creacion['nombresusuario_sistema'] ?? '');
            $sheet->setCellValue("G$row", $nombre_eliminacion['nombresusuario_sistema'] ?? '');
            $sheet->setCellValue("H$row", $detalle['justificacion'] ?? '');

            // Formato moneda
            $sheet->getStyle("E$row")->getNumberFormat()->setFormatCode('#,##0');

            $total += $detalle['valor_pedido'];
        ...
        $sheet->getStyle("D$row:E$row")->applyFromArray($headerTablaStyle);

        // =============================
        // BORDES A TODA LA TABLA
        // =============================
        $sheet->getStyle("A8:H" . ($row))
            ->applyFromArray([
                'borders' => [
                    'allBorders' => [
                        'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN
