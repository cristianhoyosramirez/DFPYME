...
<div class="modal fade" id="modalBono" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="btn-close" onclick="cleanInput()" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-outline-danger" onclick="cleanInput()">Cancelar</button>
    ...
    <script>
        function cleanInput() {
            ...
            document.getElementById('errorValor').innerText = '';
    ...
    <!--     <script>
        document.addEventListener("DOMContentLoaded", function() {
            consultarLicenciaSupabase('960caf31-47f9-4b87-aa03-37fd5c03bf74').then((resultado) => {
                if (resultado === true) {
...
<!--  Inicio consultar estado licencia -->
    <script>
        document.addEventListener("DOMContentLoaded", async function() {
            try {
                // 1. Consultar el ID de licencia desde el backend
                const respuesta = await fetch("<?= base_url('producto/idLicencia') ?>");
    ...
    <!--  Fin consultar estado licencia -->


<!--  Inicio consultar estado consumo -->
    <script>
        document.addEventListener("DOMContentLoaded", async function() {
            try {
                // 1. Consultar el ID de licencia desde el backend
                const respuesta = await fetch("<?= base_url('producto/id_licencia') ?>");
                const data = await respuesta.json();

                if (data.id_licencia) {
                    // 2. Llamar a consultarLicenciaSupabase con el ID obtenido
                    const resultado = await consultarestadoConsumo(data.id_licencia);
    ...
    <script>
        async function consultarestadoConsumo(idLicencia) {
            // const url = `https://pjhzftkkysmuwjestwsk.supabase.co/rest/v1/estado_licencia?id_cliente=eq.${idLicencia}`;
            const url = `https://pjhzftkkysmuwjestwsk.supabase.co/rest/v1/estado_consumo?id_instalacion=eq.${idLicencia}`;
            const apiKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBqaHpmdGtreXNtdXdqZXN0d3NrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDU0NTYwMzcsImV4cCI6MjA2MTAzMjAzN30.-ZLlB3hKU-Xm9_FFI29fbfVQNULWQs07QdfirZG0o0I';
            ...
            try {
                ...
                if (data.length > 0 && data[0].estado_consumo) {
                    const estado = data[0].estado_consumo;
                    const mensaje = data[0].mensaje_consumo;
                    ...
                    await fetch(url+'/impresora/actualizarEstadoConsumo', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        },
