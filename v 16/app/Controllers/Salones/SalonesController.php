...
class SalonesController extends BaseController
{
    ...
    function whatsApp()
    {
        ...
        $salonesModel = model('SalonesModel');

        $existe = $salonesModel->select('id')->where('tipo', 1)->first();
        ...
        if (!empty($existe)) {
            // Actualizar (usando la PK encontrada)
            $salonesUpdate = model('salonesModel')
                /* ->where('id', $existe['id'])
            ->update(); */
                ->set('nombre', $nombre)
                ->where('id', $existe['id'])
    ...
    public function crearMesas()
    {
        ...
        $numMesas    = $json->numMesas;
        $prefijoMesa = $json->prefijoMesa;

        // Verificar si existe un salón de tipo 1
        $salon = model('salonesModel')->select('id')->where('tipo', 1)->first();

        if (!$salon) {
            // No existe, lo creamos
            $salonId = model('salonesModel')->insert([
                'nombre' => 'Salón WhatsApp',
                'tipo'   => 1
            ], true); // true -> retorna el ID insertado
        } else {
            $salonId = $salon['id'];
        }

        // Crear las mesas para el salón obtenido/creado
        $mesas = model('salonesModel')->mesas($salonId, $prefijoMesa, $numMesas);
        ...
        if ($mesas) {
            ...
            return $this->response->setJSON([
                'response' => 'success',
                'message'  => "✅ Se crearon {$numMesas} mesas para el salón con ID {$salonId}",
            ...
            return $this->response->setJSON([
                'response' => 'error',
                'message'  => '❌ No se pudieron crear las mesas'
            ]);
        }
    }
}
