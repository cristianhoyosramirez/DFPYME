<?php foreach ($productos as $keyProducto): ?>
    <tr>
        ...
        <td>
            <?php
            $idValorUnidadMedida=model('productoMedidaModel')
            ->select('idvalor_unidad_medida')
            ->where('codigointernoproducto',$keyProducto['codigointernoproducto'])
            ->first();

            $unidadMedida = model('unidadMedidaModel')
                ->select('descripcionvalor_unidad_medida')
                ->where('idvalor_unidad_medida', $idValorUnidadMedida['idvalor_unidad_medida'])
                ->first();

            
            if (!empty($unidadMedida)) {
                echo $unidadMedida['descripcionvalor_unidad_medida'];
            }
            ?>
        </td>
        <td>
