                        ...
                        <div class="container">
                            ...
                            <div class="row container"> <!-- Fila para el textarea -->
                                ...
                                <div class="col-md-1 mb-2">
                                    ...
                                    <button class="btn btn-outline-success  btn-icon " type="button" onclick="buscarCliente()" style="height: 38px; width: 50px;">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="staticBackdropLabel">Impresoras disponibles </h1>
                                    ...
                                    <?php foreach ($impresoras as $detalleImp): ?>
                                        <div class="col-12 col-sm-6 col-md-6 col-lg-6">
                                            ...
                                            <a href="javascript:void(0)"
                                                onclick="guardarImpresora('<?= $detalleImp['id'] ?>')"
...
<div class="modal fade" id="modalBono" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    $ejecutarConsulta = ($consultar['consultar_pedidos_whatsapp'] === 't') ? 'true' : 'false';
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            // Flag proveniente de PHP
            const ejecutarConsulta = <?= $ejecutarConsulta ?>;

            if (!ejecutarConsulta) {
                console.log("La consulta de pedidos está deshabilitada desde configuración.");
            ...
            async function procesarPedidos() {
                try {
                    ...
                    const response = await fetch(url + "/whatsapp/index");
                    ...
                    if (data.procesados) {
                        data.procesados.forEach(p => {
                            console.log(`Pedido ${p.pedido}: ${p.message}`);
                        });
                    } else {
                        console.log(data.message || "Sin datos");
                    }
            ...
            procesarPedidos();
            setInterval(procesarPedidos, 15000);
        });
    </script>
