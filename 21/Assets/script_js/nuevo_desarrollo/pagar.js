...
function pagar() {
    ...
    let nit_cliente = document.getElementById("nit_cliente").value.trim();

    let forma_pago = parseInt(
        document.getElementById("formaPago").value
    );

    let valor_factura = parseFloat(
        document.getElementById("valor_total_a_pagar").value
    ) || 0;



    // VALIDAR CLIENTE PARA CRÉDITO
    if (forma_pago === 2 && nit_cliente === "222222222222") {

        Swal.fire({
            icon: 'warning',
            title: 'Cliente no válido',
            text: 'Debe seleccionar un cliente válido para realizar una venta a crédito',
    ...
    // VALIDAR VALOR FACTURA
    if (forma_pago === 2 && valor_factura <= 0) {

        Swal.fire({
            icon: 'warning',
            title: 'Valor inválido',
            text: 'El valor de la factura debe ser mayor a cero',
            showConfirmButton: false,
            timer: 2500,
            timerProgressBar: true
        });
    ...
    validarResolucion().then(data => {
        ...
        if (data.response === 'alerta' || data.response === 'false') {
            ...
            let clase_pago = document.getElementById("clase_pago").value; // Tipo de pago 1 = pago completo; 2 pago parcial
            ...
            if (transaccion > 0 && !clase_pago) {
                document.getElementById('errorClasePago').innerHTML = "Debe seleccionar un banco"
                return;
            }
                        ...
                        $.ajax({
                            ...
                            success: function (resultado) {
                                ...
                                if (resultado.resultado == 1) {
                                    ...
                                    let efectivo = document.getElementById("efectivo");
                                    let transaccion = document.getElementById("transaccion");
                                    let forma_pago = document.getElementById("formaPago");
