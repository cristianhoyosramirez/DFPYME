...
class CerrarVenta extends BaseController
{
    ...
    public function cerrar_venta()
    {
        ...
        if ($rol['idtipo'] == 1 or $rol['idtipo'] == 0  or $rol['idtipo'] == 4  or $rol['idtipo'] == 5) {
            ...
            $saldo = '';
            if ($forma_pago == 1) {  //venta de contado 
                $saldo = 0;
            }
            ...
            if ($numero_facturas['numeroconsecutivo'] <= $rango_final['rangofinaldian'] and $diferencia_fecha->format('%R%a') >= 0) {  // Se puede facturar esta bien la fecha y la numeracion
                ...
                if ($forma_pago == 2) {
                    ...
                    $temp_cambio = ($efectivo + $transaccion) - $valor_venta;
                }
                ...
                if ($tipo_pago == 0) {
                    ...
                    $returnData = array(
                        ...
                        "cambio" => "$ " . number_format($temp_cambio, 0, ",", "."),
                        "mensaje" => $mensaje,
    ...
    /*  function propinas()
    {
        ...
        $id_mesa = $this->request->getPost('id_mesa');
        //$id_mesa = 1;
        if ($calculoPropina['propina_sobre_base_tributaria'] == 't') {
            ...
            echo  json_encode($returnData);
        } else if ($calculoPropina['propina_sobre_base_tributaria'] == 'f') {
            ...
            foreach ($productos as $detalle) {
                $calculo = $impuestos->calcular_impuestos($detalle['codigointernoproducto'], $detalle['valor_total'], $detalle['valor_unitario'], $detalle['cantidad_producto']);
                 ...
                 $calculo[0]['valor_ico'],
                $calculo[0]['valor_iva'],  retorna estas dos variables necesito capturalas e irlas sumnado y ese total es el total de 
                impuesto que se le debe restar al total del pedido y esa seria la base y sobre esa base calcular la propina 
    ...
    {
        $configPropina = model('configuracionPedidoModel')
            ->select('propina_sobre_base_tributaria,valor_defecto_propina')
            ->first();
        ...
        if (!$id_mesa) {
            return json_encode(["resultado" => 0, "mensaje" => "ID de mesa no recibido"]);
        }

        $configPropina['propina_sobre_base_tributaria'];
        // Si la propina se calcula sobre TOTAL (incluye impuestos)
        if ($configPropina['propina_sobre_base_tributaria'] === 't') {

            $propinaClass = new Propina();
            $propina = $propinaClass->calcularPropina($id_mesa);
        ...
        // *************************************************************
        // CASO 2: Propina antes de impuestos (Base tributaria)
        // *************************************************************
        ...
        $pedido = model('pedidoModel')->select('id')->where('fk_mesa', $id_mesa)->first();
        if (!$pedido) {
            return json_encode(["resultado" => 0, "mensaje" => "Pedido no encontrado"]);
        }

        $productos = model('productoPedidoModel')
            ->where('numero_de_pedido', $pedido['id'])
            ->find();

        if (!$productos) {
            return json_encode(["resultado" => 0, "mensaje" => "Productos no encontrados"]);
        }
        ...
        foreach ($productos as $detalle) {
            ...
            $total_pedido += floatval($detalle['valor_total']);
            ...
            $total_iva += floatval($calculo[0]['iva']);
            $total_ico += floatval($calculo[0]['ico']);
        }

        // Impuestos totales
        $total_impuestos = $total_iva + $total_ico;



        // Base tributaria = total sin impuestos
        $base_propina = $total_pedido - $total_impuestos;


        // Porcentaje de propina desde configuración
        $porcentaje = floatval($configPropina['valor_defecto_propina']);

        // Cálculo de propina sobre la base
        $valor_propina = $base_propina * ($porcentaje / 100);

        // Total final del pedido
        $total_con_propina = $total_pedido + $valor_propina;
        ...
        $returnData = [
            ...
            "base_propina"     => number_format($base_propina, 0, ",", "."),
            "propina"          => number_format($valor_propina, 0, ",", "."),
            "valor_pedido"     => $total_pedido,
            "total_pedido"     => number_format($total_con_propina, 0, ",", "."),
            "val_pedido"       => $total_con_propina,
        ];
