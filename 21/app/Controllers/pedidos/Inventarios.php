...
class Inventarios extends BaseController
{
    ...
    public function producto_entrada()
    {
        ...
        if (!empty($resultado)) {
            foreach ($resultado as $row) {
                ...
                $data['unidad_medida'] = $row['descripcionvalor_unidad_medida'];
    ...
    function procesar_compra()
    {
        ...
        if (!empty($productos)) {
            ...
            $data = [
                ...
                'numerofactura_proveedor' => $this->request->getPost('numero_factura'),
    ...
    function consultar_compras()
    {
        ...
        $proveedores = model('ProveedorModel')->select('nombrecomercialproveedor,id,codigointernoproveedor')->orderBy('nombrecomercialproveedor', 'asc')->findAll();
    ...
    function consultar_productos_compra()
    {
       ...
       // $id = 5;
        ...
        $numeroFactura=model('FacturaCompraModel')->select('numerofactura_proveedor')->where('numeroconsecutivofactura_proveedor',$id)->first();
        /* 
        dd($productos);
        ...
        $returnData = array(
            ...
            "productos" => view('compras/productos_factura_proveedor', [
                ...
                "numero_factura"=>$numeroFactura['numerofactura_proveedor']
            ]),
        );
    ...
    function buscar_compras()
    {
        ...
        SELECT 
    fp.numeroconsecutivofactura_proveedor AS id, 
    p.nombrecomercialproveedor AS nombre_proveedor, 
    u.nombresusuario_sistema AS usuario, 
    fp.fechaingresofactura_proveedor AS fecha_ingreso,
    p.nitproveedor
FROM 
    factura_proveedor fp
INNER JOIN proveedor p 
    ON p.codigointernoproveedor = fp.codigointernoproveedor
INNER JOIN usuario_sistema u 
    ON u.idusuario_sistema = fp.idusuario_sistema
WHERE 
    fp.fechaingresofactura_proveedor BETWEEN :fecha_inicial AND :fecha_final
    AND (
        :proveedor IS NULL 
        OR :proveedor = '' 
        OR fp.codigointernoproveedor = :proveedor
    )
ORDER BY 
    fp.fechaingresofactura_proveedor DESC;
