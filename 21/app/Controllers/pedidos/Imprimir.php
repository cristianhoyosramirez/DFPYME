...
class Imprimir extends BaseController
{
    ...
    function imprimir_fiscal()
    {
        ...
        if (!empty($id_inicial[0]['id']) and !empty($id_final[0]['id'])) {
            ...
            $totalValor = $totalFormasPago[0]['total'] ?? 0;

            // Alinear usando los mismos anchos de columna
            $nombreTotal = str_pad("TOTAL FORMAS DE PAGO", $colNombre, " ", STR_PAD_RIGHT);
            $montoTotal  = str_pad(number_format($totalValor, 0, ",", "."), $colValor, " ", STR_PAD_LEFT);

            $printer->text($nombreTotal . $montoTotal . "\n");
