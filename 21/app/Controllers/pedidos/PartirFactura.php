...
class PartirFactura extends BaseController
{
    ...
    /*    function valor()
    {
        ...
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                ...
                // Solo agregamos los demás datos si resultado = 1
                if ($resultado === 1) {
                    $returnData += [
 ...
 */
    ...
    function reporteHoras()
    {
        ...
        return $this->response->setJSON([
            'response' => 'success',
            'productos' => view('consultas_y_reportes/reporteHoras', [
                'fecha' => $fecha
            ])
        ]);
    }

    function editarPrefactura()
    {
        ...
        if ($id_estado == 8) {
            $datosFactura = model('facturaElectronicaModel')->getDatosFectura($id);
        } else if ($id_estado != 8) {
            $datosFactura = model('facturaElectronicaModel')->getDatosFecturaPos($id, $id_estado);
        }
        ...
        return $this->response->setJSON([
            ...
            'editar_factura' => view(
                ...
                [
                    ...
                    'nombreCliente' => $datosFactura[0]['nombrescliente'],
                    'nitCliente' => $datosFactura[0]['nit_cliente'],
                    'medio_pago' => $datosFactura[0]['medio_pago'],
                    'metodo_pago' => $datosFactura[0]['metodo_pago'],
                    'fecha_limite' => $datosFactura[0]['fecha_limite'],
                    'id_factura' => $id

                ]

            ),
            'numero' => "Cambiar datos de la factura: " . $datosFactura[0]['numero']
        ]);
    }

    function actualizarPrefactura()
    {
        ...
        $medio_de_pago = $json->medio_de_pago;
        ...
        $actualizarF = model('facturaElectronicaModel')->set($data)->where('id', $id)->update();
        $actualizarP = model('pagosModel')->set('nit_cliente', $nit)
            ->where('id_factura', $id)
            ->where('id_estado', 8)
            ->update();
    ...
    function actualizarCantidad()
    {
        ...
        $id_tabla_producto = model('partirFacturaModel')->select('id_tabla_producto,valor_unitario')->where('id', $id)->first();

        $cantidad_producto = model('productoPedidoModel')->select('cantidad_producto')->where('id', $id_tabla_producto['id_tabla_producto'])->first();
        ...
        if ($cantidad <= $cantidad_producto['cantidad_producto']) {
            ...
            $data = [
                'cantidad_producto' => $cantidad,
                'valor_total' => $cantidad * $id_tabla_producto['valor_unitario']
            ];

            $actualizar = model('partirFacturaModel')->set($data)->where('id', $id)->update();


            $resultado = model('partirFacturaModel')->total_producto($id);

            $total_producto = $resultado[0]['total'];


            $totalPedido = model('partirFacturaModel')->selectSum('valor_total')->first();

            $total_pedido = $totalPedido['valor_total'] ?? 0;
