...
class FacturaElectronica extends BaseController
{
    ...
    function pre_factura()
    {
        ...
        $clasePago   = $this->request->getPost('clasePago');
        /* 
        if ($clasePago !== null && $clasePago !== '' && $transaccion > 0) {
        ...
        if ($validar_pedido[0]['total'] == 0) {
            ...
            $insercion =  model('facturaElectronicaModel')->insert($data);

            //var_dump($insercion); exit();
            ...
            if (($insercion == false)) {
                ...
                $returnData = array(
                    ...
                    "resultado" => 0,
                    "mensaje" => "Hay problemas relacionadas proceso del almacenamiento en base de daos pruebe limpiar el historial de navegacion, o contacte al area de soporte "
            ...
            $insert =  model('facturaElectronicaModel')->insertID();
            ...
            if ($insert) {
                ...
                if ($suma_pagos > $valor_venta) {
                    ...
                    elseif ($efectivo > 0 && $transaccion > 0) {
                        ...
                        elseif ($efectivo > $transaccion) {
                            ...
                            // Distribuir proporcionalmente el pago entre efectivo y transacción
                            $mitad = $valor_venta / 2;

                            $valor_pago_transferencia = $mitad;
                            $valor_pago_efectivo = $mitad;

                            // El cambio es la suma de ambos pagos menos el valor de la venta
                            $cambio = ($efectivo + $transaccion) - $valor_venta;
                ...
                if ($tipo_pago == 1) {
                    ...
                    if ($this->request->getPost('formaPago') == 2) {
                        ...
                        $data = [
                            ...
                            'efectivo' => 0,
                            'total_pago' => 0,
                            'recibido_transferencia' => 0,
                            'transferencia' => 0,
                            'cambio' => 0,
                            'recibido_efectivo' => 0
                        ];

                        $actualizar = model('pagosModel')->set($data)->where('id', $pagos)->update();
                    }
                ...
                if ($tipo_pago == 0) {  //pagos parcial 
                    ...
                    /*  if ($imprime_boucher['imp_comprobante_transferencia'] == 1) {
                        ...
                        if (!empty($movimientos_transaccion)) {
                            $transaccion = $movimientos_transaccion[0]['recibido_transferencia'];
                        } else if (empty($movimientos_transaccion)) {
                            $transaccion = 0;
                        }

                        if (!empty($movimientos_efectivo)) {
                            $efectivo = $movimientos_efectivo[0]['recibido_efectivo'];
                            $total_efectivo = $movimientos_efectivo[0]['total_pago'];
                        } else if (empty($movimientos_efectivo)) {
                            $efectivo = 0;
                            $total_efectivo = 0;
                        }
