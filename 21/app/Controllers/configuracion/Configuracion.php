...
class Configuracion extends BaseController
{
    ...
    function propina()
    {
        ...
        $propinaAutomatica = model('configuracionPedidoModel')->select('propina_auto')->first();
        ...
        return view('configuracion/propina', [
            ...
            'propinaAutomatica' => $propinaAutomatica['propina_auto'],
            'propinaManual' => $propinaManual['propina_manual']
        ]);
    }
    ...
    function actualizar_sub_categoria()
    {
        ...
        $categorias = model('categoriasModel')->where('permitir_categoria', 'true')->orderBy('nombrecategoria', 'asc')->findAll();
        $idCategorias = model('categoriasModel')->sub_categorias();
    ...
    function eliminar_sub_categoria()
    {
        ...
        $existe = model('productoModel')->where('id_subcategoria', $id_categoria)->findAll();
        ...
        if (empty($existe)) {
            ...
            $returnData = array(
                "resultado" => 1,
                "subcategorias" => view('configuracion/lista_sub_categorias', [
                    //"subcategorias" => view('configuracion/sub_categoria', [
                    'categorias' => $categorias,
                    'id_categorias' => $idCategorias
                ])
            ...
            $returnData = array(
                "resultado" => 0,
                "mensaje" => "No se puede eliminar la subcategoría porque tiene productos asociados."
            );
            echo json_encode($returnData);
        }
    }
...
<a href="' . $pdf_url['pdf_url'] . '" target="_blank" class="cursor-pointer">
    ...
    function comanda()
    {
        ...
        $espaciosEncabezado = model('configuracionPedidoModel')->select('espacios_comanda_encabezado')->first();
    ...
    public function eliminacion_masiva()
    {
        ...
        foreach ($facturas as $factura) {
            ...
            if ($resolucion) {
                ...
                $esCredito = model('pagosModel')->select('forma_pago')->where('id_factura', $idFactura)->where('id_estado', 8)->first();
                if ($esCredito['forma_pago'] == 1) {
                    $this->eliminarFacturaCompleta($idFactura);
                }
            }
        }
    ...
    function actualizar_tamano()
    {
        ...
        $actualizar = model('configuracionPedidoModel')->set('tamano_comanda', $tamano)->update();
    ...
    function actualizar_texto_propina()
    {

        $json = $this->request->getJSON();
        $texto = $json->texto;

        $update = model('configuracionPedidoModel')->set('texto_propina', $texto)->update();
    ...
    function propinaAutomatica()
    {

        $json = $this->request->getJSON();
        $valor = $json->valor;

        $update = model('configuracionPedidoModel')->set('propina_auto', $valor)->update();
    ...
    function propinaManual()
    {

        $json = $this->request->getJSON();
        $valor = $json->valor;

        $update = model('configuracionPedidoModel')->set('propina_manual', $valor)->update();
