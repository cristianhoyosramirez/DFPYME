    ...
    function datos_consultas_caja_por_fecha()
    {
        ...
        if (!empty($aperturas)) {
            ...
            if (empty($tiene_cierre)) {
                ...
                $diferencia =  - (($ingresos_transaccion + $ingresos_efectivo + $valor_apertura['valor']+$total_abonos) - ($retiros + $devoluciones));
            }
            if (!empty($tiene_cierre)) {
                ...
                $diferencia =  $cierre_usuario - (($total_ingresos+$total_abonos) - $retiros_dinero);
            }


            //$abonos=model('facturaFormaPagoModel')->where('id_apertura',$ultimo_id)->findAll();
            ...
            return view('consultas_y_reportes/datos_consultas_caja', [
                ...
                'ingresos_efectivo' =>  "$" . number_format(($ingresos_efectivo + $ingresos_transaccion + $total_abonos) - $propinas, 0, ",", "."),
                'ingresos_transaccion' =>  "$" . number_format($ingresos_transaccion, 0, ",", "."),
                'total_ingresos' =>  "$" . number_format(($ingresos_transaccion + $ingresos_efectivo + $total_abonos) + $valor_apertura['valor'], 0, ",", "."),
                ...
                'saldo_caja' => "$" . number_format(($valor_apertura['valor'] + $ingresos_efectivo + $ingresos_transaccion + $total_abonos) - ($retiros + $devoluciones), 0, ",", "."),
