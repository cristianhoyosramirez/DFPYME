...
class Ventas extends BaseController
{
    ...
    public function ventas()
    {
        ...
        $movimientos = model('pagosModel')
            ->select('pagos.*, cliente.nitcliente, cliente.nombrescliente')
        ...
        $cortesias = model('pagosModel')
            ->selectSum('total_documento', 'total')
            ->where('id_apertura', $id_apertura)
            ->where('id_estado', 6)
            ->first()['total'] ?? 0;

        $ventas_credito = model('pagosModel')
            ->selectSum('total_documento', 'total')
            ->where('id_apertura', $id_apertura)
            ->where('forma_pago', 2)
            ->first()['total'] ?? 0;

        $ventas_contado = model('pagosModel')
            ->selectSum('total_documento', 'total')
            ->where('id_apertura', $id_apertura)
            ->where('forma_pago', 1)
            ->first()['total'] ?? 0;

        //$abonos = model('facturaFormaPagoModel')->where('id_apertura', $id_apertura)->findAll();

        $abonos = model('facturaFormaPagoModel')
            ->select("
        factura_forma_pago.*,
        CASE
            WHEN factura_forma_pago.id_clase_pago = 0 THEN 'Efectivo'
            ELSE clase_pago.nombre
        END AS nombre_clase_pago
    ")
            ->join(
                'clase_pago',
                'clase_pago.id = factura_forma_pago.id_clase_pago',
                'left'
            )
            ->where('factura_forma_pago.id_apertura', $id_apertura)
            ->findAll();

        //dd($abonos);

        $total_abonos = model('facturaFormaPagoModel')
            ->selectSum('valor_pago', 'total_abonos')
            ->where('id_apertura', $id_apertura)
            ->first();

        $total_abonos = $total_abonos['total_abonos'] ?? 0;
        ...
        $returnData = array(
            ...
            "total_ingresos" => number_format(($transferencia[0]['transferencia'] + $efectivo[0]['efectivo']) + $total_abonos, 0, ",", "."),
            ...
            "total_abonos" => number_format($total_abonos, 0, ",", "."),
            "cortesias" => number_format($cortesias, 0, ",", "."),
            "ventas_credito" => number_format($ventas_credito, 0, ",", "."),
            "ventas_contado" => number_format($ventas_contado, 0, ",", "."),
            "total_ventas" => number_format($ventas_contado+$ventas_credito, 0, ",", "."),
    ...
    public function exportar_excel()
    {
        ...
        $fechaApertura = model('aperturaModel')
            ->select('fecha')
            ->where('id', $id_apertura)
            ->first();

        $fechaCierre = model('cierreModel')
            ->select('fecha')
        ...
        $cierre = !empty($fechaCierre)
            ? $fechaCierre['fecha']
            : 'No se ha cerrado caja';
    ...
    ")
        ...
        $file_name = 'Movimiento_Caja.xlsx';
        ...
        $sheet->setCellValue('E8', 'Nit');
        $sheet->setCellValue('F8', 'Cliente');
        $sheet->setCellValue('G8', 'Valor venta');
        $sheet->setCellValue('H8', 'Propina');
        $sheet->setCellValue('I8', 'Venta + propina');
        $sheet->setCellValue('J8', 'Efectivo');
        $sheet->setCellValue('K8', 'Transferencia');
        $sheet->setCellValue('L8', 'Total medio pago');
        $sheet->setCellValue('M8', 'Clase pago');
        $sheet->setCellValue('N8', 'Forma pago');
        $sheet->setCellValue('O8', 'Usuario');
        ...
        foreach ($movimientosCaja as $valor) {


            $sheet->setCellValue('A' . $row, $valor['tipo']);
            $sheet->setCellValue('B' . $row, $valor['fecha']);
            $sheet->setCellValue(
                'C' . $row,
                date('h:i:s A', strtotime($valor['hora']))
            );
            $sheet->setCellValue('D' . $row, $valor['documento']);

            //$sheet->setCellValue('E' . $row, $valor['nit_cliente']);
            ...
            $sheet->setCellValue('F' . $row, $valor['nombre_cliente']);
            $sheet->setCellValue('G' . $row, $valor['venta']);
            $sheet->setCellValue('H' . $row, $valor['propina']);
            $sheet->setCellValue('I' . $row, $valor['total']);
            $sheet->setCellValue('J' . $row, $valor['efectivo']);
            $sheet->setCellValue('K' . $row, $valor['transferencia']);
        ...
        $ventas_electronicas = model('pagosModel')
            ->set_ventas_electronicas($id_apertura);
        ...
        $transferencia = model('pagosModel')
            ->selectSum('transferencia')
            ->where('id_apertura', $id_apertura)
            ->first();

        $efectivo = model('pagosModel')
            ->selectSum('efectivo')
            ->where('id_apertura', $id_apertura)
            ->first();
        ...
        $sheet->setCellValue(
            'B' . $row,
            $ventas_electronicas[0]['valor'] ?? 0
        );

        $row++;

        $sheet->setCellValue('A' . $row, 'Propinas');
        $sheet->setCellValue(
            'B' . $row,
            $propinas['propina'] ?? 0
        );
        ...
        $sheet->setCellValue('A' . $row, 'EFECTIVO');

        $sheet->setCellValue(
            'B' . $row,
            $total_efectivo['efectivo'] ?? 0
        );
    ...
    public function pedidos_borrados()
    {
        ...
        foreach ($datos as $detalle) {
            ...
            $sub_array[] = $detalle['justificacion'];
    ...
    public function exportarExcel()
    {
        ...
        $ruta = 'C:/Users/Usuario/Desktop/JSON';
        ...
        foreach ($archivos as $archivo) {
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                log_message('error', "JSON inválido en: $archivo - " . json_last_error_msg());
                continue;
            }

            // No trae usuarios
            if (!isset($data['usuarios']) || !is_array($data['usuarios'])) {
                log_message('error', "Archivo sin usuarios: $archivo");
                continue;
            }
            ...
            foreach ($data['usuarios'] as $usuario) {
                ...
                if (isset($servicios['medicamentos'])) {
                    foreach ($servicios['medicamentos'] as $item) {
                        $rows[] = $this->flattenRow($data, $usuario, $item, 'medicamento');
                    }
                }

                if (isset($servicios['otrosServicios'])) {
                    foreach ($servicios['otrosServicios'] as $item) {
                        $rows[] = $this->flattenRow($data, $usuario, $item, 'otrosServicio');
                    }
                }

                if (isset($servicios['consultas'])) {
                    foreach ($servicios['consultas'] as $item) {
                        $rows[] = $this->flattenRow($data, $usuario, $item, 'consulta');
                    }
                }
            }
        }
        ...
        // 🔐 TODAS LAS COLUMNAS en formato TEXTO
        $sheet->getStyle('A:ZZ')->getNumberFormat()
        ...
        // Filas con texto real (sin comillas)
        ...
        // Guardar archivo
        $filename = 'export_json_total.xlsx';
    ...
    private function flattenRow(array $data, array $usuario, array $item, string $tipo): array
    {
        $row = [];

        // Datos generales del JSON (menos usuarios)
        foreach ($data as $k => $v) {
            if ($k === 'usuarios') continue;
            $row[$k] = $this->safeValue($v);
        }

        // Datos del usuario
        foreach ($usuario as $k => $v) {
            if ($k === 'servicios') continue;
            $row["usuario_" . $k] = $this->safeValue($v);
        }

        // Datos del item (servicio)
        foreach ($item as $k => $v) {
            $row[$tipo . "_" . $k] = $this->safeValue($v);
        }
    ...
    private function safeValue($value)
    {
        ...
        // 👇 SI ES NÚMERO LARGO, DEVOLVER COMO STRING SIN COMILLA
