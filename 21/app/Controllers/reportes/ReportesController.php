...
class ReportesController extends BaseController
{
   ...
   "actions": {
    ...
    public function actualizar_pagos()
    {
        ...
        $transferencia = (int) preg_replace('/\D/', '', $transferencia);
        ...
        if ($totalPago > $totalDocumento) {
            ...
            return $this->response->setJSON([
                'status' => 'error',
                'mensaje' => 'El valor ingresado excede el total de la factura por $' . number_format($excedente, 0, ',', '.')
            ]);
        }
        ...
        if ($totalPago < $totalDocumento) {
            ...
            return $this->response->setJSON([
                'status' => 'error',
                'mensaje' => 'El pago es insuficiente. Faltan $' . number_format($faltante, 0, ',', '.')
            ]);
        }
    ...
    function datos_pagos()
    {
        ...
        $returnData = array(
            "resultado" => 1,
            "forma_pago" => view('formas_pago/editar_forma_pago', [
                'efectivo' => number_format($pagos[0]['efectivo'], 0, ",", "."),
                "transferencia" => number_format($pagos[0]['transferencia'], 0, ",", "."),
                "clase_pago" => $pagos[0]['id_clase_pago'],
                "tipos_pago" => $clasePago
            ]),
            "id" => $id,
            "nit_cliente" => $pagos[0]['nit_cliente'],
            "nombre_cliente" => $pagos[0]['nombrescliente'],
            "nombre_pago" => $nombre_pago,
            "total" => number_format($pagos[0]['total_documento'], 0, ",", "."),
            'efectivo' => number_format($pagos[0]['efectivo'], 0, ",", "."),
            "transferencia" => number_format($pagos[0]['transferencia'], 0, ",", "."),
            "total_pagado" => number_format($pagos[0]['transferencia'] + $pagos[0]['efectivo'], 0, ",", ".")

        );
        echo  json_encode($returnData);
    }

    function datos_abono()
    {
    ...
    //$id = 135;  
        ...
        $id_fact = model('pagosModel')->select('id')->where('id_factura', $datos['id_factura'])
        ...
        $returnData = array(
            ...
            "nit_cliente" => $pagos[0]['nit_cliente'],
            "nombre_cliente" => $pagos[0]['nombrescliente'],
            "numero_factura" => $pagos[0]['numero_factura'],
            "fecha_factura" => $pagos[0]['fecha'],
            "total_factura" =>  number_format($pagos[0]['total_documento'], 0, ",", "."),
            "saldo" => number_format($pagos[0]['saldo'], 0, ",", "."),
            "total" => number_format($datos['valor_pago'], 0, ",", "."),
            'efectivo' => number_format($datos['efectivo'], 0, ",", "."),
            "transferencia" => number_format($datos['electronico'], 0, ",", "."),
            "id_clase_pago" => $datos['id_clase_pago']
