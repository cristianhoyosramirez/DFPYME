...
class Boletas extends BaseController
{
    ...
    function actualizar_producto_porcentaje()
    {
        ...
        $nuevoPrecio = str_replace('.', '', $this->request->getPost('nuevoPrecio'));
    ...
    /*         public function tipo_documento()
{
    ...
    public function consultar_entradas()
    {
        ...
        foreach ($datos as $detalle) {
            ...
            $sub_array[] = $detalle['numerofactura_proveedor'];
    ...
    public function consultar_compras()
    {
        ...
        $proveedor    = $request->getPost('proveedor');


        // 🧠 MAPEO COLUMNAS
        $table_map = [
            0 => 'numeroconsecutivofactura_proveedor',
            1 => 'fechaingresofactura_proveedor',
            2 => 'nitproveedor',
            3 => 'nombresusuario_sistema',
            4 => 'nombrecomercialproveedor',
        ];

        $orderBy = $table_map[$orderColumn] ?? 'numeroconsecutivofactura_proveedor';

        // 🧱 BASE SQL
        $base_sql = "
        FROM factura_proveedor
        INNER JOIN proveedor 
            ON proveedor.codigointernoproveedor = factura_proveedor.codigointernoproveedor
        INNER JOIN usuario_sistema 
            ON usuario_sistema.idusuario_sistema = factura_proveedor.idusuario_sistema
    ...
    ";
        ...
        // 📅 FILTRO FECHA (CORRECTO PARA DATETIME)
        if (!empty($fecha_inicio) && !empty($fecha_fin)) {
            $base_sql .= " 
            where fechaingresofactura_proveedor BETWEEN ' $fecha_inicio' AND ' $fecha_fin' 
        ";
        }


        // 🏢 FILTRO PROVEEDOR (OPCIONAL Y CORREGIDO)
        /*  if (isset($proveedor) && $proveedor !== '' && $proveedor !== '0') {
            $base_sql .= " AND factura_proveedor.codigointernoproveedor = ?";
            $params[] = (int)$proveedor;
        } */

        if (isset($proveedor) && $proveedor !== '' && $proveedor !== '0') {
            $base_sql .= " 
        AND factura_proveedor.codigointernoproveedor = $proveedor
    ";
        }
        // 🔍 BUSCADOR GLOBAL
        if (!empty($valor_buscado)) {
            $base_sql .= " AND (
            proveedor.nitproveedor ILIKE ? OR
            proveedor.nombrecomercialproveedor ILIKE ? OR
            factura_proveedor.numerofactura_proveedor ILIKE ?
        )";

            $searchValue = "%$valor_buscado%";
            $params[] = $searchValue;
            $params[] = $searchValue;
            $params[] = $searchValue;
        }
        ...
        $total_count = $this->db->query($sql_count, $params)->getRow();
    ...
    ";
        ...
        $params_data = array_merge($params, [$length, $start]);

        $datos = $this->db->query($sql_data, $params_data)->getResultArray();
        ...
        foreach ($datos as $detalle) {
            ...
            $data[] = [
                ...
                $detalle['numerofactura_proveedor'],
                $detalle['nombre_proveedor'],
                $detalle['nitproveedor'],
                number_format($total[0]['total_compra'] ?? 0, 0, ',', '.'),
                $detalle['usuario'],
                '
            <a class="btn btn-outline-success btn-icon" title="Imprimir" onclick="imprimir_compra(' . $detalle['id'] . ')">🖨️</a>
            <a class="btn btn-outline-secondary btn-icon" title="Detalle" onclick="detalle_compra(' . $detalle['id'] . ')">👁️</a>
            '
            ];
        }
