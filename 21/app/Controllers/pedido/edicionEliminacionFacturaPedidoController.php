...
class edicionEliminacionFacturaPedidoController extends BaseController
{
    ...
    function consultarFe()
    {
        ...
        $facturarPedidos = model('configuracionPedidoModel')->select('facturacion')->first();
        ...
        if ($facturarPedidos['facturacion'] == 't' and  !empty($hayPedidos)) {   // en este caso se deben de facturar todos los pedidos 
            ...
            return $this->response->setJSON([
                'response' => 'fail',
                'message'  => 'No se puede cerrar la caja por que hay pedidos pendientes por facturar .'
            ]);
        }
        ...
        if ($facturarPedidos['facturacion'] == 'f') {  // se puede facturar sin importar si hay o no pedidos pendientes por facturar 
            // Buscar apertura activa
            $temp_id_apertura = model('aperturaRegistroModel')->select('id, numero')->first();
    ...
    function facturacion()
    {
        ...
        $configuracion = model('configuracionPedidoModel')->select('facturar_cero,borrarFe')->first();
        ...
        return view('menu/facturacion', [
            ...
            'prefactura' => $configuracion['borrarFe'],
        ]);
    }
