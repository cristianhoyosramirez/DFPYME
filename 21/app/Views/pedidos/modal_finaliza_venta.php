                ...
                <form>
                  ...
                  <div class="col-sm-12">
                    ...
                    <div class="input-group mb-3">
                      ...
                      <select class="form-select" id="formaPago" name="formaPago" onchange="desabilitar_opciones()">
...
<script>
  /**
   * --------------------------------------------------------------------------
   * FUNCIÓN: desabilitar_opciones()
   * --------------------------------------------------------------------------
   * Esta función controla el comportamiento de los campos de pago
   * dependiendo de la forma de pago seleccionada.
   *
   * FUNCIONAMIENTO:
   *
   * 1. Obtiene la forma de pago seleccionada.
   * 2. Obtiene los inputs:
   *      - efectivo
   *      - transaccion
   *      - valor_total_a_pagar
   *
   * 3. Cuando la forma de pago es CRÉDITO (valor = 2):
   *
   *      - El campo "efectivo" SIEMPRE toma el valor total a pagar.
   ...
   *            * si es mayor a cero se coloca en 0
   *
   *      - Finalmente se garantiza nuevamente
   *        que el campo efectivo conserve
   *        el valor total a pagar.
   *
   * 4. Cuando la forma de pago es CONTADO:
   *
   *      - Los campos vuelven a habilitarse.
   *      - El efectivo sigue mostrando
   *        el valor total a pagar.
   *
   * --------------------------------------------------------------------------
   */

  function desabilitar_opciones() {
    ...
    // INPUT TOTAL A PAGAR
    let valor_total_a_pagar = document.getElementById("valor_total_a_pagar").value;
    ...
    */
    if (formaPago == "2") {
      ...
      /*
      |--------------------------------------------------------------------------
      | EL EFECTIVO SIEMPRE TENDRÁ EL TOTAL A PAGAR
      |--------------------------------------------------------------------------
      */
      efectivo.value = valor_total_a_pagar;
      ...
      */
      ...
      |--------------------------------------------------------------------------
      */
      let valorTransaccion = transaccion.value
        .replace(/\./g, '')
        .replace(/,/g, '');
      ...
      /*
      |--------------------------------------------------------------------------
      | VALIDAR SI TRANSACCIÓN ES MAYOR A 0
      |--------------------------------------------------------------------------
      */
      if (parseFloat(valorTransaccion) > 0) {

        /*
        |--------------------------------------------------------------------------
        | SI TIENE VALOR, SE COLOCA EN 0
        |--------------------------------------------------------------------------
        */
        transaccion.value = 0;
      }
      ...
      /*
      |--------------------------------------------------------------------------
      | GARANTIZAR NUEVAMENTE EL VALOR DEL EFECTIVO
      |--------------------------------------------------------------------------
      */
      efectivo.value = valor_total_a_pagar;
    }
    ...
    */
      ...
      */
      ...
      transaccion.disabled = false;


      /*
      |--------------------------------------------------------------------------
      | MANTENER EL TOTAL A PAGAR EN EFECTIVO
      |--------------------------------------------------------------------------
      */
      efectivo.value = valor_total_a_pagar;
    }
