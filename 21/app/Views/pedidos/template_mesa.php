...
<body>
    ...
    <div class="modal fade" id="modalModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <script src="<?= base_url() ?>/Assets/script_js/descuentoPorcentaje/confirmarDescuento.js"></script>
    <script src="<?= base_url() ?>/Assets/script_js/descuentoPorcentaje/restablecerPrecio.js"></script>



    <!-- Modal -->
    <div class="modal fade" id="notaCliente" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Buscar datos de cliente </h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="buscarCliente" class="form-label">Buscar por nombre, identificación o teléfono</label>
                        <input type="text" class="form-control ui-autocomplete-input" id="buscarClienteNota" placeholder="Escribe aquí...">
                    </div>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-succes" data-bs-dismiss="modal">Aceptar</button>
    ...
    <script>
        ...
        $("#buscarClienteNota").autocomplete({
            source: function(request, response) {
                var url = document.getElementById("url").value;
                $.ajax({
                    type: "POST",
                    url: url + "/clientes/clientes_autocompletadoNota",
                    data: request,
                    success: response,
                    dataType: "json",
                });
            },
            appendTo: "#notaCliente",
            minLength: 1,
            select: function(event, ui) {
                $("#nota_pedido").val(ui.item.datos).trigger("input"); // dispara oninput

                $(this).val("");
                $('#notaCliente').modal('hide');
    ...
    <script>
        ...
        function seleccionarComponentes(nombreComponente, componenteId, numeroComponentes, atributoId) {
            ...
            // Filtrar los ya seleccionados de este mismo atributo
            const seleccionadosPorAtributo = componentesSeleccionados.filter(comp => comp.atributoId === atributoId);

            if (seleccionadosPorAtributo.length >= numeroComponentes) {
                sweet_alert_centrado('warning', `Solo puedes seleccionar ${numeroComponentes} componente(s) para este atributo`);
                return;
            }

            // Validar que no se repita el componente
        ...
        function renderizarBotonesComponentes() {
            // Limpio todos los contenedores primero
            document.querySelectorAll("[id^='contenedorAtributo']").forEach(div => div.innerHTML = '');

            componentesSeleccionados.forEach((item, index) => {
                const contenedor = document.getElementById(`contenedorAtributo${item.atributoId}`);
    ...
    <script>
        let mensaje = <?= json_encode($user_session->getFlashdata('mensaje')) ?>;
        let iconoMensaje = <?= json_encode($user_session->getFlashdata('iconoMensaje')) ?>;
        let estado = <?= json_encode(session()->getFlashdata('estado')) ?>;
        let titulo = <?= json_encode(session()->getFlashdata('titulo')) ?>;

        if (mensaje) {
            Swal.fire({
                //title: 'Estado de consumo: \n ' + estado,
                title: titulo,
                text: mensaje,
                icon: iconoMensaje.toLowerCase(),
                ...
                success: function(resultado) {
                    ...
                    if (resultado.resultado == 1) {
                        ...
                        mesas.forEach(function(mesa) {
                            ...
                            let card = `
    <div class="cursor-pointer card card_mesas text-white bg-red-lt"
         onclick="pedido_mesa('${mesa.fk_mesa}','${mesa.nombre}')"
         style="height:auto;">
        <div class="row">
            <div class="col-3">
                <span class="avatar">
                    <img src="${url}/Assets/img/ocupada.png"
                         width="110"
                         height="32"
    ...
    <script>
        function calcular_porcentaje(valorPorcentaje) {
            var url = document.getElementById("url").value;
            var id_producto_pedido = document.getElementById("id_producto_pedido").value;
            var id_usuario = document.getElementById("id_usuario").value;
            var precio = document.getElementById("valorReferencia").value;

            var descuento = precio * (valorPorcentaje / 100);
            var precioFinal = Math.round(precio - descuento); // Redondeado sin decimales

            // Formatear con separador de miles
            var precioFormateado = precioFinal.toLocaleString('es-CO'); // Ej: "1.234.567"

            document.getElementById("precio_producto").value = precioFormateado;
    ...
    <!--    <script>
        function calcular_porcentaje(valorPorcentaje) {
            ...
            var id_producto_pedido = document.getElementById("id_producto_pedido").value;
            var id_usuario = document.getElementById("id_usuario").value;
            var precio = parseFloat(document.getElementById("valorReferencia").value) || 0;

            // Validar si el porcentaje es vacío, cero o no numérico
            if (!valorPorcentaje || isNaN(valorPorcentaje) || parseFloat(valorPorcentaje) <= 0) {
                document.getElementById("precio_producto").value = ""; // o "0"
                return;
            }

            var porcentaje = parseFloat(valorPorcentaje);
            var descuento = precio * (porcentaje / 100);
            var precioFinal = Math.round(precio - descuento);

            var precioFormateado = precioFinal.toLocaleString('es-CO');
            document.getElementById("precio_producto").value = precioFormateado;
        }
    </script> -->
