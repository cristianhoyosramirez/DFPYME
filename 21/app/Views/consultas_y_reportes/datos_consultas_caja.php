            ...
            <div class="modal-body">
                ...
                <form id="formRetiros">
                    ...
                    <div class="row">
                        <div class="col-8"></div>
                        <div class="col-2">
                            <button type="button" class="btn btn-outline-success" onclick="actualizarRetiro()">Aceptar</button>
                        </div>
                        <div class="col-2">
                            <button type="button" class="btn btn-outline-danger" data-bs-dismiss="modal">Cancelar</button>
                        </div>
...
<!-- Modal Editar Pagos -->
<div class="modal fade" id="editar_pagos" data-bs-backdrop="static" data-bs-keyboard="false"
    tabindex="-1" aria-labelledby="editarPagosLabel" aria-hidden="true">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-header bg-light">
                <h5 class="modal-title" id="editarPagosLabel">
                    <i class="bi bi-cash-coin"></i> Editar pagos de factura
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            ...
            <div class="modal-body">
                ...
                <form action="<?= base_url('reportes/actualizar_pagos') ?>"
                    method="POST"
                    id="frmPago">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="nit_cliente" readonly>
                            </div>
                    ...
                    <!-- ================= DATOS DE LA FACTURA ================= -->
                    <div class="mb-4">
                        <h6 class="text-muted mb-3">Información de pago factura</h6>

                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label">Efectivo</label>
                                <input type="text" class="form-control" id="valor_efectivo_factura" readonly>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    <!-- Valor registrado en la factura -->
                                    <input type="text"
                                        class="form-control"
                                        id="valor_transferencia_factura"
                                        readonly
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="valor_total_factura" readonly>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- ================= FORMAS DE PAGO ================= -->
                    <div class="mb-4">
                        <h6 class="text-muted mb-3">Modificar las formas de pago</h6>
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-4">
                        <button type="button" onclick="validarFormulario()" id="btnFrmPago" class="btn btn-outline-success">
...
<!-- Modal -->
<div class="modal fade" id="modal_editar_abono" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">Cambio de formas de pago </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="hidden" id="id_abono_edicion">

                        <input
                            type="text"
                            class="form-control"
                            id="editar_numero_factura"
                            readonly>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input
                            type="text"
                            class="form-control formato-dinero-abono"
                    ...
                    <div class="col-md-4">
                        ...
                        <?php
                        $clase_pago = model('clasePagoModel')
                            ->where('estado', 't')
                            ->findAll();
                        ?>
                        ...
                        <select
                            ...
                            <?php foreach ($clase_pago as $detalleCp): ?>

                                <option value="<?= $detalleCp['id'] ?>">
                                    <?= esc($detalleCp['nombre']) ?>
                                </option>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-danger" data-bs-dismiss="modal">Cancelar</button>
                <button type="button" class="btn btn-outline-success" onclick="editarAbono()">Guardar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    async function editarAbono() {

        try {

            const response = await fetch('<?= base_url('consultas_y_reportes/editarAbonar') ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    efectivo: $('#nuevo_efectivo').val(),
                    transferencia: $('#nueva_transferencia').val(),
                    id: $('#id_abono_edicion').val(),
                    banco: $('#banco').val()
                })
            });
            ...
            if (resultado.success == true) {
                ...
                const respuesta = await Swal.fire({
                    ...
                    showDenyButton: true,
                    denyButtonText: '🖨️ Imprimir comprobante',
                    customClass: {
                ...
                if (respuesta.isDenied) {
                    ...
                    try {
                        ...
                        const response = await fetch("<?= base_url('consultas_y_reportes/imprimir_comprobante_ingreso') ?>", {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'No fue posible guardar el abono'
            });
...
<script>
    function limpiar_edicion_abono() {

        $('#nuevo_efectivo').val('');
        $('#nueva_transferencia').val('');
        $('#texto_total_nuevo').html('$0');
...
<script>
    function formatearMil(input) {
        ...
        let efectivo = document.getElementById('nuevo_efectivo').value.replace(/\./g, '');
        let transferencia = document.getElementById('nueva_transferencia').value.replace(/\./g, '');

        efectivo = parseInt(efectivo, 10) || 0;
        transferencia = parseInt(transferencia, 10) || 0;

        let total = efectivo + transferencia;



        const totalPagado = document.getElementById('texto_total_nuevo');

        if (totalPagado) {
            totalPagado.innerHTML = new Intl.NumberFormat('es-CO').format(total);
        } else {
            console.error('No existe el elemento con id total_pagado');
        }
    }
</script>
...
<script>
    function validarSelect(valor) {
        ...
        let select = document.getElementById('forma_pago');


        if (valor) {
            // Opción válida seleccionada → quitar error
...
<!-- <script>
    function validarFormulario() {
        ...
        let transaccion = document
            .getElementById('transferencia_factura')
            .value
            .replace(/\D/g, '');

        let forma_pago = document.getElementById('forma_pago');

        if (transaccion > 0 && !forma_pago.value) {
...
<script>
    async function validarFormulario() {
        ...
        let transaccion = document.getElementById('transferencia_factura')?.value.replace(/\D/g, '') || 0;
        let forma_pago = document.getElementById('forma_pago');

        if (parseInt(transaccion) > 0 && (!forma_pago || !forma_pago.value)) {
            forma_pago.classList.add('is-invalid');
            forma_pago.focus();
            ...
            /*      Swal.fire({
                     icon: 'warning',
                     title: 'Atención',
                     text: 'Debes seleccionar un banco si hay transferencia.'
                 }); */
        ...
        try {
            ...
            Swal.fire({
                ...
                // Cerrar modal
                let modal = bootstrap.Modal.getInstance(document.getElementById('editar_pagos'));
                if (modal) modal.hide();

                // Aquí puedes recargar tabla o refrescar datos
...
<script>
    async function actualizarRetiro() {
        ...
        try {
            const respuesta = await fetch("<?php echo base_url() ?>/devolucion/actualizar_retiro", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            if (resultado.status === 'success') {
                Swal.fire({
                    icon: 'success',
                    title: 'Éxito',
                    text: resultado.mensaje || 'Retiro actualizado correctamente',
                    showConfirmButton: false,
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: resultado.mensaje || 'No se pudo actualizar el retiro.'
                });
            }
        ...
        $.ajax({
            ...
            success: function(resultado) {
                ...
                if (resultado.resultado == 1) {
                    ...
                    $('#total_abonos').html(resultado.total_abonos)

                    $('#cortesias').html(resultado.cortesias)
                    $('#ventas_credito').html(resultado.ventas_credito)
                    $('#ventas_contado').html(resultado.ventas_contado)
                    $('#abonos').html(resultado.total_abonos)
