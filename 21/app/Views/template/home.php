    ...
    <div class="wrapper">
        ...
        <script>
            ...
            function cargarTabla() {
                ...
                // 🔥 Si ya existe, destruir correctamente
                if ($.fn.DataTable.isDataTable('#consulta_compras')) {
                    $('#consulta_compras').DataTable().destroy();
                }

                tabla = $('#consulta_compras').DataTable({
                    serverSide: true,
                    processing: true,
                    searching: true,
                    order: [
                    ...
                    ajax: {
                        url: '<?= base_url() ?>/eventos/consultar_compras',
                        type: "POST",

                        // 🔥 CLAVE: mantener estructura DataTables + tus filtros
                        data: function(d) {
                            d.fecha_inicio = $('#fecha_inicial').val();
                            d.fecha_fin = $('#fecha_final').val();
                            d.proveedor = $('#id_proveedor').val();
                            d.buscar_por = $('#buscar_por').val();
                        },
                        ...
                        dataSrc: function(json) {
                            ...
                            $('#saldo').html(json.saldo_pendiente_por_cobrar ?? 0);
                            $('#abonos').html(json.abonos ?? 0);
                            $('#total_documentos').html(json.total ?? 0);
                            $('#total_ventas').html(json.titulo ?? '');
                            $('#dian_no_enviado').html(json.dian_no_enviado ?? 0);
                            $('#dian_aceptado').html(json.dian_aceptado ?? 0);
                            $('#dian_rechazado').html(json.dian_rechazado ?? 0);
                            $('#dian_error').html(json.dian_error ?? 0);

                            $('#pagos_recibidos').toggle((json.abonos_sin_punto ?? 0) > 0);
                            $('#saldo_pendiente_pago').toggle((json.saldo_pendiente_por_cobrar_sin_punto ?? 0) > 0);
        ...
        <script>
            async function actualizar_factura(id) {
                try {

                    nit = document.getElementById('nit_del_cliente').value
                    id = document.getElementById('id_de_factura').value
                    formaPago = document.getElementById('formaPagoEdit').value
                    fechaLimite = document.getElementById('fechaLimiteEdit').value
                    medio_de_pago = document.getElementById('medio_de_pagoEdit').value

                    const response = await fetch("<?= base_url('pedidos/actualizarPrefactura') ?>", {
                        method: 'POST',
                        headers: {
                    ...
                    if (data.response == "success") {
                        ...
                        sweet_alert_centrado('success', 'Datos cambiados ')
                    ...
                    if (data.response == "success") {
                        document.getElementById('datosFactura').innerHTML = data.editar_factura
                        document.getElementById('tituloEditarFactura').innerHTML = data.numero


                        $('#edicionFactura').modal('show')
                    }
	...
	<span class="text-red"> <svg xmlns="http://www.w3.org/2000/svg" class="icon " width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M4 9h8v-3.586a1 1 0 0 1 1.707 -.707l6.586 6.586a1 1 0 0 1 0 1.414l-6.586 6.586a1 1 0 0 1 -1.707 -.707v-3.586h-8a1 1 0 0 1 -1 -1v-4a1 1 0 0 1 1 -1z" /></svg></span>`;
        ...
        <script>
            function estado_dian(id_estado) {
                ...
                var tablaVentas = $('#consulta_ventas').DataTable({
                    serverSide: true,
                    processing: true,
                    searching: true,
                    order: [
        ...
        <script>
            function buscar() {
                ...
                var nit_cliente = document.getElementById("nit_del_cliente").value;
        ...
        <script>
            $("#busqueda_de_cliente").autocomplete({
                source: function(request, response) {
                    var url = document.getElementById("url").value;
                    $.ajax({
                        type: "POST",
                ...
                select: function(event, ui) {
                    // $("#id_cliente_factura_pos").val(ui.item.value);
                    //$("#clientes_factura_pos").val(ui.item.nit_cliente);
                    $("#nit_del_cliente").val(ui.item.nit_cliente);
                    $("#busqueda_de_cliente").val(ui.item.value);
                    //$("#busqueda_de_cliente").val(ui.item.value);
        ...
        <script>
            $("#busqueda_por_cliente").autocomplete({
                source: function(request, response) {
                    var url = document.getElementById("url").value;
                    $.ajax({
                        type: "POST",
                ...
                select: function(event, ui) {
                    ...
                    $("#nit_del_cliente").val(ui.item.nit_cliente);
                    //$("#busqueda_de_cliente").val(ui.item.value);
                    $("#busqueda_por_cliente").val(ui.item.value);
