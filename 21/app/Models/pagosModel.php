...
WHERE 
    ...
    public function getMedioPago($idFactura)
    {
        $datos = $this->db->query("
            
   SELECT 
    mp.nombre_comercial AS nombre_medio_pago
FROM documento_electronico AS de
INNER JOIN medio_pago AS mp
    ON de.medio_pago = mp.codigo
WHERE de.id = $idFactura;
...
WHERE id_factura = $idFactura
    ...
    public function pagos_realizados($idFactura)
    {
        $datos = $this->db->query("
            
                SELECT 
                    factura_forma_pago.valor_pago,
                    clase_pago.nombre, 
                    idfactura_forma_pago
                FROM factura_forma_pago
