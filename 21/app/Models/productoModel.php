...
AND id_tipo_inventario NOT IN (3)
   ...
   public function autoCompletePro($valor)
    ...
    p.aplica_descuento,
    p.precio_costo,
    vm.idvalor_unidad_medida,
    vm.descripcionvalor_unidad_medida
FROM
    public.producto p
INNER JOIN categoria c ON p.codigocategoria = c.codigocategoria
LEFT JOIN producto_medida pm ON p.codigointernoproducto = pm.codigointernoproducto
LEFT JOIN valor_unidad_medida vm ON vm.idvalor_unidad_medida = pm.idvalor_unidad_medida
WHERE
    p.id_tipo_inventario IN (1, 4)
    AND p.id_tipo_inventario NOT IN (3)
    AND p.estadoproducto = 'true'
    AND (
        p.codigobarrasproducto ILIKE '%$valor%' 
        OR p.codigointernoproducto ILIKE '%$valor%' 
        OR p.nombreproducto ILIKE '%$valor%'
    );
...
WHERE (id_tipo_inventario = 3 OR id_tipo_inventario = 7)
  AND (nombreproducto ILIKE '%$valor%' OR codigointernoproducto ILIKE '%$valor%');
    ...
    function GetInsumos($valor)
    {
        ...
        $datos = $this->db->query("
       SELECT codigointernoproducto, nombreproducto,precio_costo,valorventaproducto ,id_tipo_inventario FROM producto WHERE (
  nombreproducto ilike '%$valor%'
  OR
  codigointernoproducto ilike '%$valor%'
)
AND
id_tipo_inventario= 1
OR
id_tipo_inventario= 4
OR
id_tipo_inventario= 7 ;
