...
<body>
    <input type="text" value="<?php echo base_url() ?>" id="url" hidden>
    <div>
            ...
            <table class="table-numeric" id="formulario">
                <tbody>
                    <tr>
                        ...
                        <td><button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('1')">1</button></td>
                        <td><button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('2')">2</button></td>
                        <td><button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('3')">3</button></button></td>
                    </tr>
                    <tr>

                        <td><button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('4')">4</button></td>
                        <td><button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('5')">5</button></td>
                        <td><button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('6')">6</button></button></td>
                    </tr>
                    <tr>

                        <td><button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('7')">7</button></td>
                        <td><button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('8')">8</button></td>
                        <td><button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('9')">9</button></button></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td> <button type="button" class="btn btn-outline-success py-3 px-3" onclick="agregarDigito('0')">0</button></td>
...
<script>
    ...
    function login() {
        ...
        if (input.length === 4) {
            // Aquí llamas a la función real de login
            alert("Código enviado: " + input);
        } else {
...
<script>
    let mensaje = <?= json_encode(session()->getFlashdata('mensaje')) ?>;
    let iconoMensaje = <?= json_encode(session()->getFlashdata('iconoMensaje')) ?>;
    ...
    if (mensaje) {
        Swal.fire({
            title: estado,
            text: mensaje,
            icon: iconoMensaje ? iconoMensaje.toLowerCase() : 'info',
...
<!-- <script>
    <?php if (session()->getFlashdata('mensaje')): ?>
        Swal.fire({
            title: '<?= session()->getFlashdata('estado') ?>',
            text: '<?= session()->getFlashdata('mensaje') ?>',
            imageUrl: '<?= session()->getFlashdata('imagenMensaje') ?>',
            imageWidth: 100,
            imageHeight: 100,
            imageAlt: 'Alerta personalizada'
        });
    <?php endif; ?>
</script> -->
