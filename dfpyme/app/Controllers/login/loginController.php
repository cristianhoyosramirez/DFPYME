    ...
    public function login()
    {
        ...
        if (!empty($usuario)) {
            ...
            if ($usuario) {
                ...
                /* if ($usuario[0]['idtipo'] == 0 or $usuario[0]['idtipo'] == 1 or $usuario[0]['idtipo'] == 3 or $usuario[0]['idtipo'] == 4 or $usuario[0]['idtipo'] == 5) {

                    if ($estadoLicencia['estado_licencia'] == "En Mora") {
                        $mensajeLicencia = model('licenciaModel')->select('mensaje_licencia')->first();
                        $session = session();
                    ...
                    if ($estadoLicencia['estado_licencia'] == "Activa") {

                        return redirect()->to(base_url('pedidos/mesas'));
                    }

                    if ($estadoLicencia['estado_licencia'] == "Suspendida" or $estadoLicencia['estado_licencia'] == "Suspendida") {
                        $mensajeLicencia = model('licenciaModel')->select('mensaje_licencia')->first();
                        $session = session();
                        $session = session();
                        $session->setFlashdata('iconoMensaje', 'warning');
                        return redirect()->to(base_url())->with('mensaje', $mensajeLicencia['mensaje_licencia']);
                    }
                } */
                ...
                if (in_array($usuario[0]['idtipo'], [0, 1, 3, 4, 5])) {
                    ...
                    if ($estado == "En Mora") {
                        ...
                        //$mensajeLicencia['mensaje_licencia']; exit();
                        $session = session();
                        $session->setFlashdata('iconoMensaje', 'warning');
                        return redirect()->to(base_url('pedidos/mesas'))->with('mensaje', $mensajeLicencia['mensaje_licencia']);
                    }
        ...
        /* } else if ($estadoLicencia['estado'] == 'f') {

            $session = session();
            $session->setFlashdata('iconoMensaje', 'Error');
            return redirect()->to(base_url())->with('mensaje', 'Licencia no presenta pago ');
        } */
    }
