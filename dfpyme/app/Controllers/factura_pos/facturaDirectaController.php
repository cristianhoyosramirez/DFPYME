...
class facturaDirectaController extends BaseController
{
    ...
    public function facturacion()
    {
        ...
        if ($valor_venta_sin_punto <= $total_pagado_sin_punto) {
            ...
            switch ($id_regimen['idregimen']) {
                case 1:
                    $this->facturacion_con_impuestos($fecha_y_hora, $saldo, $efectivo_sin_punto, $transaccion_sin_punto, $valor_venta_sin_punto, $nit_cliente, $id_usuario, $estado, $total_pagado_sin_punto, $descuento, $propina);
                    break;
                case 2:
                    $this->facturacion_sin_impuestos($fecha_y_hora, $saldo, $efectivo_sin_punto, $transaccion_sin_punto, $valor_venta_sin_punto, $nit_cliente, $id_usuario, $estado, $total_pagado_sin_punto, $descuento, $propina);
                    break;
            }
    ...
    public function facturacion_sin_impuestos($fecha_y_hora, $saldo, $efectivo_sin_punto, $transaccion_sin_punto, $valor_venta_sin_punto, $nit_cliente, $id_usuario, $estado, $total_pagado_sin_punto, $descuento, $propina)
    {
        ...
        $fk_mesa = model('mesasModel')->select('id')->where('fk_usuario', $id_usuario)->first();
        ...
        if ($estado == 1 or $estado == 2) {
            $numero_de_factura =  $numero_factura['numeroconsecutivo'];
        }

        if ($prefijo_factura != 0) {
            $data = [
                'numerofactura_venta' => $numero_de_factura,
                //'numerofactura_venta' => $prefijo_factura['numeroconsecutivo'] . "-" . $numero_factura['numeroconsecutivo'],
            ...
            if ($efectivo_sin_punto > 0 and $transaccion_sin_punto == 0) {
                ...
                if ($insert_efectivo) {
                    ...
                    $returnData = array(
                        ...
                        "id_regimen" => 2,
                        "cambio" => number_format($efectivo_sin_punto - (($total['valor_total'] - $descuento) + $propina), 0, ",", "."),
            ...
            if ($efectivo_sin_punto > 0 and $transaccion_sin_punto > 0) {
                ...
                if ($insert_efectivo and $insert_transaccion) {
                    ...
                    $returnData = array(
                        ...
                        "id_regimen" => 2,
                        "cambio" => number_format(($efectivo_sin_punto + $transaccion_sin_punto) - (($total['valor_total'] - $descuento) + $propina), 0, ",", "."),
            ...
            if ($efectivo_sin_punto == 0 and $transaccion_sin_punto > 0) {
                ...
                if ($insert_transaccion) {
                    ...
                    $returnData = array(
                        "id_factura" => $id_factura,
                        "resultado" => 1,
                        "efectivo" => number_format($transaccion_sin_punto, 0, ",", "."),
                        "id_regimen" => 2,
                        "cambio" => number_format($transaccion_sin_punto - (($total['valor_total'] - $descuento) + $propina), 0, ",", "."),
                        "total" => number_format($total['valor_total'], 0, ",", "."),
                        "id_usuario" => $id_usuario
    ...
    public function facturacion_con_impuestos($fecha_y_hora, $saldo, $efectivo_sin_punto, $transaccion_sin_punto, $valor_venta_sin_punto, $nit_cliente, $id_usuario, $estado, $total_pagado_sin_punto, $descuento, $propina)
    {
