...
class facturaDirectaController extends BaseController
{
    ...
    public function ventasHora()
    {
        ...
        // Título reporte
        $sheet->setCellValue("A$row", "VENTAS POR HORA - " . $fechaInicial);
        $sheet->mergeCells("A{$row}:E{$row}");
        ...
        for ($hora = 0; $hora < 24; $hora++) {
            ...
            $productos_hora = model('reporteProductoModel')->getProductos($inicio, $fin, $fechaInicial);
            $total_hora = model('reporteProductoModel')->getTotal($inicio, $fin, $fechaInicial);

            if (!empty($productos_hora)) {
                $sheet->setCellValue("A$row", "{$inicio} - {$fin}");
                $sheet->getStyle("A$row")->getFont()->setBold(true);
                $row++;

                foreach ($productos_hora as $producto) {
                    $sheet->setCellValue("B$row", $producto['codigo']);
                    $sheet->setCellValue("C$row", $producto['nombreproducto']);
                    $sheet->setCellValue("D$row", $producto['total_cantidad']);
                    $sheet->setCellValue("E$row", $producto['total_valor']);
                    $row++;
                }

                // Total por hora
                $sheet->setCellValue("A$row", "TOTAL {$inicio} - {$fin}");
                $sheet->mergeCells("A{$row}:D{$row}");
                $sheet->setCellValue("E$row", $total_hora[0]['total_valor']);
        ...
        // Descargar archivo
        $nombreArchivo = 'Ventas_por_hora_' . $fechaInicial . '.xlsx';
        $writer = new Xlsx($spreadsheet);
