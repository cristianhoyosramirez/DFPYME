...

{
    ...
    public function cerrar_venta()
    {
        ...
        if ($total_pagado >= $valor_venta) {
            ...
            switch ($id_regimen['idregimen']) {
                case 1:
                    $this->facturacion_con_impuestos($fecha_y_hora, $numero_de_pedido, $efectivo, $transaccion, $valor_venta, $nit_cliente, $id_usuario, $estado, $total_pagado, $descuento, $propina, $result, $resultado);
                    break;
                case 2:
                    $this->facturacion_sin_impuestos($fecha_y_hora, $numero_de_pedido, $efectivo, $transaccion, $valor_venta, $nit_cliente, $id_usuario, $estado, $total_pagado, $descuento, $propina);
                    break;
            }
    ...
    public function facturacion_sin_impuestos($fecha_y_hora, $numero_pedido, $efectivo, $transaccion, $valor_venta, $nit_cliente, $id_usuario, $estado, $total_pagad, $descuento, $propina)
    {
        $saldo = '';
        ...
        if ($efectivo > 0 and $transaccion == 0) {
            ...
            if ($insert_efectivo) {
                ...
                $returnData = array(
                    ...
                    "id_regimen" => 1,
                    "cambio" => number_format($efectivo - $total['valor_total'], 0, ",", "."),
                    "total" => number_format(($total['valor_total'] - $descuento) + $propina, 0, ",", "."),
                    "descuento" => $descuento,
                    "propina" => $propina
        ...
        if ($efectivo > 0 and $transaccion > 0) {
            ...
            $efecti_trasacc = $efectivo + $transaccion;

            $insert_efectivo = model('facturaFormaPagoModel')->insert($factura_forma_pago_efectivo);
            $transaccion = model('facturaFormaPagoModel')->insert($factura_forma_pago_transaccion);
            $id_mesa = model('pedidoModel')->select('fk_mesa')->where('id', $numero_pedido)->first();
            $id_usuario = model('pedidoModel')->select('fk_usuario')->where('id', $numero_pedido)->first();
            if ($insert_efectivo) {
                $data = [
                ...
                $returnData = array(
                    "id_factura" => $id_factura,
                    "resultado" => 1,
                    "efectivo" => number_format($efecti_trasacc, 0, ",", "."),
                    "id_regimen" => 1,
                    //"cambio" => number_format(($efectivo+$transaccion) - $total['valor_total'], 0, ",", "."),
                    "cambio" => number_format($efecti_trasacc - $total['valor_total'], 0, ",", "."),
                    "total" => number_format(($total['valor_total'] - $descuento) + $propina, 0, ",", "."),
                    "descuento" => $descuento,
                    "propina" => $propina
        ...
        if ($efectivo == 0 and $transaccion > 0) {
            ...
            if ($insert_transaccion) {
                ...
                $returnData = array(
                    "id_factura" => $id_factura,
                    "resultado" => 1,
                    "efectivo" => number_format($transaccion, 0, ",", "."),
                    "id_regimen" => 1,
                    //"cambio" => number_format(($efectivo+$transaccion) - $total['valor_total'], 0, ",", "."),
                    "cambio" => number_format($transaccion - $total['valor_total'], 0, ",", "."),
                    "total" => number_format(($total['valor_total'] - $descuento) + $propina, 0, ",", "."),
                    "descuento" => $descuento,
                    "propina" => $propina
    ...
    public function facturacion_con_impuestos($fecha_y_hora, $numero_pedido, $efectivo, $transaccion, $valor_venta, $nit_cliente, $id_usuario, $estado, $total_pagado, $descuento, $propina, $result, $resultado)
    {
        $saldo = '';
        ...
        if ($efectivo > 0 and $transaccion > 0) {
            ...
            if ($insert_efectivo and $insert_transaccion) {
                ...
                $returnData = array(
                    ...
                    "resultado" => 1,
                    "efectivo" => number_format($efectivo, 0, ",", "."),
                    "cambio" => number_format(($efectivo + $transaccion) - $valorfactura_forma_pago['valor_total'], 0, ",", "."),
                    //"total" =>  number_format($valorfactura_forma_pago['valor_total'], 0, ",", "."),
                    "id_regimen" => 2,
                    "Sub_total" => number_format($sub_totales, 0, ",", "."),
                    "iva" => number_format($total_iva, 0, ",", "."),
        ...
        if ($efectivo == 0 and $transaccion >= 0) {
            ...
            if ($insert_transaccion) {
                ...
                $returnData = array(
                    ...
                    "resultado" => 1,
                    "efectivo" => number_format($efectivo, 0, ",", "."),
                    "cambio" => number_format($transaccion - $valorfactura_forma_pago['valor_total'], 0, ",", "."),
                    //"total" =>  number_format($valorfactura_forma_pago['valor_total'], 0, ",", "."),
                    "id_regimen" => 2,
                    "Sub_total" => number_format($sub_totales, 0, ",", "."),
                    "iva" => number_format($total_iva, 0, ",", "."),
