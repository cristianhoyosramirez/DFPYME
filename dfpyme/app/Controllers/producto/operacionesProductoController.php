...
class operacionesProductoController extends BaseController
{
        ...
        if (
            ...
            if ($this->request->getPost('informacion_tributaria') == 1) {
                ...
                $data = [
                    ...
                    'grupo_impresion_comanda' => $this->request->getPost('grupoImpresion')
                ];
    ...
    function editar_precios()
    {
        ...
        $grupo_impresion = model('productoModel')->select('grupo_impresion_comanda')->where('codigointernoproducto', $id_producto)->first();
        ...
        $sub_categorias = model('subCategoriaModel')->where('id_categoria', $id_categoria['codigocategoria'])->findAll();
