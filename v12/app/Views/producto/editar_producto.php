<div class="hr-text hr-text-center mb-3">
...
<form class="row g-1" action="<?= base_url('producto/actualizar_precio_producto'); ?>" method="post" id="cambiar_datos_de_producto" autocomplete="off">
    ...
    <div class="row mb-3">
        ...
        <div class="col-2">
            ...
            <select name="UnidadMedida" id="UnidadMedida" class="form-select">
                <?php foreach ($unidadMedida as $unidad): ?>
                    <option value="<?php echo $unidad['idvalor_unidad_medida']; ?>"
                        <?php echo ($UnidadMedida == $unidad['idvalor_unidad_medida']) ? 'selected' : ''; ?>>
        ...
        <div class="col-2">
            <label for="">Grupo impresión</label>
            <?php $grupoImpresion = model('grupoImpresionModel')->findAll(); ?>
            <select name="grupoImpresionEdicion" id="grupoImpresionEdicion" class="form-select">
                <?php foreach ($grupoImpresion as $grupoImpresiones): ?>
                    <option value="<?php echo $grupoImpresiones['id']; ?>"
                        <?php echo ($grupoImpresiones['id'] == $grupo_impresion) ? 'selected' : ''; ?>>
                        <?php echo $grupoImpresiones['nombre']; ?>
                    </option>
                <?php endforeach ?>
            </select>
