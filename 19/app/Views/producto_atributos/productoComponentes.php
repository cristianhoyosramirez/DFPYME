...
<?php foreach ($idAtributos as $keyAtributo):
    $nombre = model('atributosProductoModel')
        ->select('nombre')
        ->where('id', $keyAtributo['id_atributo'])
        ->first();

    $numeroComponentes = model('configuracionAtributosProductoModel')
        ->getNumeroComponentes($idProducto, $keyAtributo['id_atributo']);

    $id = model('configuracionAtributosProductoModel')
        ->getId($idProducto, $keyAtributo['id_atributo']);
?>
        ...
        <div class="scroll-container">
            <?php foreach ($componentes as $detalleComponentes): ?>
                <button type="button"
                    ...
                    onclick="seleccionarComponentes(
                    '<?php echo $detalleComponentes['nombre']; ?>',
                    <?php echo $detalleComponentes['id']; ?>,
                    <?php echo $numeroComponentes[0]['numero_componentes']; ?>,
                    <?php echo $keyAtributo['id_atributo']; ?>)">
                    <?php echo htmlspecialchars($detalleComponentes['nombre'], ENT_QUOTES, 'UTF-8'); ?>
                </button>
            <?php endforeach; ?>
        </div>

        <!-- Contenedor donde se pegarán los botones seleccionados de este atributo -->
        <div id="contenedorAtributo<?php echo $keyAtributo['id_atributo']; ?>" class="mt-2"></div>
